from utils.get_api_data import *
...
def line_chart_energy(kpi):
    
    df = get_eurostat_api_data("sdg_07_40", 'nat')
...
def bar_chart_TPA_prot_area():
    
    df = get_eurostat_api_data("env_bio4", 'nat')
...
def line_chart_waste_recycled():

    df = get_eurostat_api_data("env_wastrt", 'nat')
...
def donut_chart_waste_recycled():
...
def line_chart_economic_sustainability(kpi):
...
def bar_chart_economic_sustainability(kpi):
...
def line_chart_social_sustainability(kpi):
    
    if kpi in ["urb_clivcon", "urb_ceduc"]:
        df = get_eurostat_api_data(kpi, 'nuts3')
    ...
    if kpi in ["hlth_cd_yro", "hlth_cd_yinfr"]:
        ...
        if kpi == "hlth_cd_yro":
            ...
            option = {
                ...
                'series': [{'name': 'A. M. Lisboa',
                'type': 'line',
                'data': [93.0, 88.0, 94.0, 96.0, 107.0, 106.0, 96.0, 82.0],
                'itemStyle': {'color': '#FF79C6'}},
                {'name': 'Helsinki-U.',
                'type': 'line',
                'data': [33.0, 28.0, 30.0, 30.0, 31.0, 30.0, 29.0, 29.0],
                'itemStyle': {'color': '#8BE9FD'}},
                {'name': 'Ile de France',
                'type': 'line',
                'data': [663.0, 682.0, 679.0, 708.0, 712.0, 708.0, 688.0, 678.0],
                'itemStyle': {'color': '#BD93F9'}},
                {'name': 'Köln',
                'type': 'line',
                'data': [121.0, 129.0, 140.0, 155.0, 157.0, 157.0, 147.0, 135.0],
                'itemStyle': {'color': '#6272A4'}},
                {'name': 'S. Slovensko',
                'type': 'line',
                'data': [62.0, 57.0, 49.0, 43.0, 43.0, 51.0, 55.0, 55.0],
                'itemStyle': {'color': '#FFB86C'}}]}
    
    return st_echarts(options=option, height="500px")


def bar_chart_social_sustainability(kpi):
